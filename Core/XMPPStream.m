...
- (BOOL)connectBosh:(NSError **)errPtr
{
    ...
    {
        [transport setMyJID:myJID_setByClient];
    }
...
- (BOOL)authenticateWithPassword:(NSString *)inPassword error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		{		
            ...
            NSXMLElement *auth1 = [NSXMLElement elementWithName:@"auth" xmlns:@"urn:ietf:params:xml:ns:xmpp-sasl"];
            [auth1 addAttributeWithName:@"mechanism" stringValue:@"PLAIN"];
            [auth1 setStringValue:base64];
            
            [transport sendStanza:auth1];
...
- (void)handleAuth1:(NSXMLElement *)response
{
	if([self supportsDigestMD5Authentication])
	{
		...
		else
		{
			...
			//XMPPDigestMD5Authentication *auth = [[XMPPDigestMD5Authentication alloc] 
              ///                                   initWithStream:self password:password];
...
- (void)transport:(id<XMPPTransportProtocol>)sender didReceiveStanza:(NSXMLElement *)element
{
	...
	else
	{
		if([elementName isEqualToString:@"iq"])
		{
			...
			while([delegateEnumerator getNextDelegate:&delegate delegateQueue:&(xmppQueue) forSelector:selector])
			{
...
{
    [multicastDelegate xmppStream:self didReceiveError:nil];
}
