...
NSString *const XMPPBoshStreamErrorDomain = @"XMPPStreamErrorDomain";
...
@interface XMPPBoshStream (PrivateAPI)
...
- (void)commonInit
{
	...
	multicastDelegate = (GCDMulticastDelegate <XMPPBoshStreamDelegate> *)[[GCDMulticastDelegate alloc] init];
	
	state = STATE_DISCONNECTED;
	
	registeredModules = [[GCDMulticastDelegate alloc] init];
...
- (id)initWithTransport:(id<XMPPTransportProtocol>)givenTransport
{
    if ((self = [super init]))
    {
        ...
        BoshTransport *b = (BoshTransport*)transport;
        [b aDelegate:self queue:xmppQueue];
    }
    return self;
}
...
- (BOOL)connect:(NSError **)errPtr
{
	if (state > STATE_DISCONNECTED)
	{
		if (errPtr)
		{
			...
			*errPtr = [NSError errorWithDomain:XMPPBoshStreamErrorDomain code:XMPPBoshStreamInvalidState userInfo:info];
		}
		return NO;
	}
	...
	if (myJID == nil)
	{
		...
		if (errPtr)
		{
			...
			*errPtr = [NSError errorWithDomain:XMPPBoshStreamErrorDomain code:XMPPBoshStreamInvalidProperty userInfo:info];
		}
		return NO;
	}

    // Notify delegates
    [multicastDelegate xmppBoshStreamWillConnect:self];
...
- (void)disconnect
{
	...
	[multicastDelegate xmppBoshStreamWasToldToDisconnect:self];
...
- (BOOL)secureConnection:(NSError **)errPtr
{
	...
	if (![self supportsStartTLS])
	{
		if (errPtr)
		{
			...
			*errPtr = [NSError errorWithDomain:XMPPBoshStreamErrorDomain code:XMPPBoshStreamUnsupportedAction userInfo:info];
		}
		return NO;
	}
...
- (void)sendElement:(NSXMLElement *)element withTag:(long)tag
{
	...
	{
		[multicastDelegate xmppBoshStream:self willSendIQ:(XMPPIQ *)element];
	}
	else if ([element isKindOfClass:[XMPPMessage class]])
	{
		[multicastDelegate xmppBoshStream:self willSendMessage:(XMPPMessage *)element];
	}
	else if ([element isKindOfClass:[XMPPPresence class]])
	{
		[multicastDelegate xmppBoshStream:self willSendPresence:(XMPPPresence *)element];
	}
	else
	{
		...
		{
			[multicastDelegate xmppBoshStream:self willSendIQ:[XMPPIQ iqFromElement:element]];
		}
		else if ([elementName isEqualToString:@"message"])
		{
			[multicastDelegate xmppBoshStream:self willSendMessage:[XMPPMessage messageFromElement:element]];
		}
		else if ([elementName isEqualToString:@"presence"])
		{
			[multicastDelegate xmppBoshStream:self willSendPresence:[XMPPPresence presenceFromElement:element]];
		}
	}
	...
	if ([element isKindOfClass:[XMPPIQ class]])
	{
		[multicastDelegate xmppBoshStream:self didSendIQ:(XMPPIQ *)element];
	}
	else if ([element isKindOfClass:[XMPPMessage class]])
	{
		[multicastDelegate xmppBoshStream:self didSendMessage:(XMPPMessage *)element];
	}
	else if ([element isKindOfClass:[XMPPPresence class]])
	{
		...
		[multicastDelegate xmppBoshStream:self didSendPresence:(XMPPPresence *)element];
	}
}
...
{
    [multicastDelegate xmppBoshStreamDidSecure:self];
    [self restartStream];
}

- (void)transportWillDisconnect:(id<XMPPTransportProtocol>)sender withError:(NSError *)err
{
    [multicastDelegate xmppBoshStream:self didReceiveError:err];
}
...
- (void)transportDidDisconnect:(id<XMPPTransportProtocol>)sender
{
    ...
    [multicastDelegate xmppBoshStreamDidDisconnect:self];
}
...
- (void)registerModule:(XMPPModule *)module
{
	...
	//GCDMulticastDelegateEnumerator *delegatesEnumerator = [delegates delegateEnumerator];
	//id delegate;
	...
	[multicastDelegate xmppBoshStream:self didRegisterModule:module];*/
}
...
- (void)unregisterModule:(XMPPModule *)module
{
	...
	[multicastDelegate xmppBoshStream:self willUnregisterModule:module];
...
- (id)initWithChallenge:(NSXMLElement *)challenge
{
	if((self = [super init]))
	{
		...
		// Generate cnonce
		cnonce = [[XMPPBoshStream generateUUID] retain];
	}
	return self;
}
